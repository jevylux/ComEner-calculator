...
parser.add_argument(
    '-sm', '--sendmails',
    action='store_true',
    help='if set, mails are sended to the consumers and producers with the results of the calculations'
)
...
for producer in production:
    ...
    if not items:
        print(f"⚠️ No data returned for Producer {producer['name']} with OBIS {obiscode}. Skipping.")
        continue
...
for consumer in consumption:
    ...
    items = json_data.get('items', [])
     ...
     # Only process if there is data
    if not items:
        print(f"⚠️ No data returned for Consumer {consumer['name']} with OBIS {obiscode}. Skipping.")
        continue
    df = pd.DataFrame(response.json())
    df['value'] = df['items'].apply(lambda x: x['value'])
    df['startedAt'] = df['items'].apply(lambda x: pd.to_datetime(x['startedAt']))
...
# here we will send emails to producers and comsumers with the results of the calculations

if sendMails:
    with open('./configs/mailSecrets.yaml', 'r') as file:
        mailServerData = yaml.safe_load(file)
    print("Sending mails to consumers and producers")

    msg = EmailMessage()
    msg.set_content(f"this is a text with a variable {decomptefilename}")

        # me == the sender's email address
        # you == the recipient's email address
    msg['Subject'] = f'Decompte  {args.year} - {args.month}'
    msg['From'] = mailServerData['data']['mailuser']
    msg['To'] = "marc@durbach.com"

    mailserver = smtplib.SMTP_SSL(mailServerData['data']['mailserver'],mailServerData['data']['mailport'])
        # identify ou
        # rselves to smtp gmail client
    mailserver.ehlo()
        # secure our email with tls encryption
        # re-identify ourselves as an encrypted connection
    mailserver.ehlo()
    mailserver.login(mailServerData['data']['mailuser'],mailServerData['data']['mailpassword'])

    mailserver.sendmail("marc@durbach.com",'jevylux@gmail.com',msg.as_string())
